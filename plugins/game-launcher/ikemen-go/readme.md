# @roster-lock/game-launcher-ikemen-go

A `game-launcher` plugin that launches [Ikemen GO](https://github.com/ikemen-engine/Ikemen-GO)
...
Whoever called `startGame` (in practice: match-agent's own `GameLauncher.startGame`, see
`core/plugin-runtime/src/GameLauncher.ts`) took a `ConnectionSetup` carrying a
...
`GameLauncherPlugin.getSupportedVersion` takes no `target` - the answer doesn't vary by
...
`GameLauncherPlugin` already handles as its own first-class concept (passed directly to

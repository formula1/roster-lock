...
function buildLockWithPieceMeta(): RosterLockV1Config {
  ...
  lock.selection.piece["character"] = {
    ...
    pieceMeta: {
      schema: { power: "number" },
      defaultMeta: { power: 1 },
      pieceMeta: { "@author/blue": { power: 5 } },
    },
  };
  return lock;
}

describe("selection piece meta validation", () => {
  ...
  it("rejects default meta missing a required schema key", () => {
    const lock = buildLockWithPieceMeta();
    lock.selection.piece["character"].pieceMeta.defaultMeta = {};
  ...
  it("rejects per-piece meta for a piece that isn't in the roster", () => {
    const lock = buildLockWithPieceMeta();
    lock.selection.piece["character"].pieceMeta.pieceMeta = { "@author/missing": { power: 2 } };

  ...
  if(!scriptRef){
    return (
      <>
        ...
        <h3>
          <Link
            to={RosterLockPaths.Selection.ScriptDictionary.AddScripts}
          >Add Scripts</Link>
        </h3>
      </>
    )
...
export function ScriptRunner(
  ...
  const runResult = useRunnable(
    useCallback(async ()=>{
      if(!plugins) throw new Error("No agent connected to run scripts");
      return plugins.runScript(
        {
          config: lock,
          purpose: purposeBody,
    ...
    }, [plugins, lock, purposeBody, seedPrefix, script])
  )

  if(!plugins){
    return (
      <div className="section">
        <div className="error">Connect a match-agent to run scripts</div>
      </div>
    );
  }

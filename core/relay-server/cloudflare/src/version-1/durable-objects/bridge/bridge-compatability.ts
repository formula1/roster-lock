...
type Handlers = {
  ...
  // by returning true once every machine's in) - the return value is threaded
  ...
  response: Record<string, (room: RoomType, machine: WebSocket, value: any)=>Promise<boolean | void>>,
  event: Record<string, (room: RoomType, machine: WebSocket, value: any)=>Promise<void>>,
  request: Record<string, (room: RoomType, machine: WebSocket, value: any)=>Promise<any>>,
};

export async function handleBridgeMessage(
  room: RoomType, handlers: Handlers, machine: WebSocket, messageRaw: string
): Promise<boolean> {
  ...
  if(message.messageType === "event"){
    ...
    await handler(room, machine, message.value);
    return false;
  }
  ...
  if(message.messageType === "request"){
    ...
    try {
      const response = await handler(room, machine, message.value);
  ...
  if(message.messageType === "response"){
    ...
    const request = await room.state.storage.transaction(async (txn) => {
      ...
      if(request.publicKey !== publicKey) throw new Error("Invalid machine");
      await txn.delete(`ws-request-${id}`);
      return request;
    });
    const handler = handlers.response[request.path];
    if(!handler) throw new Error("Invalid request path");
    const finished = await handler(room, machine, value);
    return !!finished;
  }
...
export async function makeBridgeRequest(room: RoomType, machine: WebSocket, path: string, value: any){
  const publicKey = getPublicKey(machine);
...
export async function makeBridgeEvent(room: RoomType, machine: WebSocket, path: string, value: any){
  machine.send(JSON.stringify({
    messageType: "event",
    path,
    value

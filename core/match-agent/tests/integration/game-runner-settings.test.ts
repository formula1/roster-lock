...
// Mirrors plugins/game-runner/ikemen-go/src/binaryLocation.ts's own mapping -
...
describe("game-runner local settings/version/update routes", () => {
  ...
  it("resolves a relative binaryLocation against pluginDir (docs/v2/binary-location.md)", async () => {
    ...
    const res = await fetch(`${server.httpUrl}/v1/game-runner/${encodeURIComponent(PLUGIN_NAME)}/validate`, {
      headers: auth(server),
    });
    expect(res.status).toBe(200);

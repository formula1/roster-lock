...
const MATCH_AGENT_URL = process.env.MATCH_AGENT_URL ?? "http://localhost:58732";
const AUTH_CODE = process.env.MATCH_AGENT_AUTH ?? "abc123";
const MATCHMAKER_URL = process.env.PUBLIC_MATCHMAKER_URL ?? "http://localhost:7344";
const GAME_COORDINATOR_URL = process.env.PUBLIC_GAME_COORDINATOR_URL ?? "http://localhost:7345";
 ...
 *   PWA_URL, MATCH_AGENT_URL, MATCH_AGENT_AUTH, PUBLIC_MATCHMAKER_URL,
 *   DISPLAY_NAME, CHROME_PATH, ROSTER_LOCK_PATH
 */
...
async function main() {
  ...
  try {
    ...
    await page.fill("#url", MATCH_AGENT_URL);
    await page.fill("#authCode", AUTH_CODE);
    await page.fill("#matchmakerUrl", MATCHMAKER_URL);
    await page.fill("#gameCoordinatorUrl", GAME_COORDINATOR_URL);

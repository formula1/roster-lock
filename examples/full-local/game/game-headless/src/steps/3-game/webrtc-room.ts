...
import {
  RTCPeerConnection, RTCIceCandidate, RTCDataChannel, RTCPeerConnectionIceEvent
} from "@roamhq/wrtc";
type RTCSessionDescriptionInit = string;
...
function setupWebRTCPeers(bridge: MessageBridge, allowedUsers: Array<string>){
  ...
  const bufferedIce: Record<string, Array<typeof RTCIceCandidate>> = {};
...
class WebRTCBridge extends MessageBridge {
  constructor(private peer: Omit<WebRTCPeer, "bridge">){
    ...
    peer.datachannel.addEventListener("message", (message: MessageEvent)=>{

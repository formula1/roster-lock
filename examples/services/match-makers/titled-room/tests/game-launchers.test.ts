...
import { assertGameLauncherAllowed, gameCoordinatorFor } from "../src/game-launchers";
...
function seedConfig(db: FakeAdminD1, pluginName: string, engineSha: string, coordinator: Record<string, unknown> = {}) {
  db.gameLauncherConfigs.set(pluginName, {
...
describe("assertGameLauncherAllowed", () => {
  it("rejects when no config row exists (fail closed)", async () => {
    ...
    await expect(
      assertGameLauncherAllowed(env, PLUGIN_NAME, { engine: { pieceDefinitions: PIECE_DEFINITIONS } })
  ...
  it("rejects a plugin not present in the config table", async () => {
    ...
    await expect(
      assertGameLauncherAllowed(makeEnv(db), PLUGIN_NAME, { engine: { pieceDefinitions: PIECE_DEFINITIONS } })
    ).rejects.toThrow(/not allowed/);
  });
  ...
  it("rejects a rosterConfig missing engine.pieceDefinitions", async () => {
    ...
    await expect(assertGameLauncherAllowed(makeEnv(db), PLUGIN_NAME, {})).rejects.toThrow(/pieceDefinitions/);
  });
});
...
describe("gameCoordinatorFor", () => {
  it("throws when no config row exists (missing entry is a misconfiguration)", async () => {
    const env = makeEnv(new FakeAdminD1());
    await expect(gameCoordinatorFor(env, PLUGIN_NAME)).rejects.toThrow(/No game-launcher config/);
  });
  ...
  it("throws for a plugin not present in the config table", async () => {
    ...
    await expect(gameCoordinatorFor(makeEnv(db), PLUGIN_NAME)).rejects.toThrow(/No game-launcher config/);
  });

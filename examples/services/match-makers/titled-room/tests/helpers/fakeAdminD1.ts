...
// admin/jwt.ts, admin/game-launchers.ts and game-launchers.ts issue against the
...
export class FakeAdminD1 {
  ...
  gameLauncherConfigs = new Map<string, Record<string, unknown>>();
...
class FakeAdminStatement {
  ...
  async first<T>(): Promise<T | null> {
    ...
    if (this.sql.includes("FROM game_runner_configs")) {
      ...
      return (this.db.gameLauncherConfigs.get(pluginName) as T) ?? null;
    }
  ...
  async run() {
    ...
    if (this.sql.includes("INSERT INTO game_runner_configs")) {
      ...
      this.db.gameLauncherConfigs.set(plugin_name as string, {
    ...
    if (this.sql.includes("DELETE FROM game_runner_configs")) {
      const [pluginName] = this.args as [string];
      const existed = this.db.gameLauncherConfigs.delete(pluginName);
  ...
  async all<T>() {
    if (this.sql.includes("FROM game_runner_configs")) {
      return { results: [...this.db.gameLauncherConfigs.values()] as Array<T> };
    }

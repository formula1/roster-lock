...
async function getGameLauncherConfig(env: Env, pluginName: string): Promise<GameLauncherConfigRow | null> {
  return env.DB.prepare(
    "SELECT * FROM game_runner_configs WHERE plugin_name = ?"
  ).bind(pluginName).first<GameLauncherConfigRow>();
}

// A room may only be created for a game-launcher plugin an admin has
// explicitly allowed (see src/admin/game-launchers.ts, which manages the
...
export async function assertGameLauncherAllowed(
...
// assertGameLauncherAllowed reads. Every allowlisted plugin must have an
// explicit coordinator choice recorded when an admin adds it (see
// src/admin/game-launchers.ts) - `false` is a deliberate "this plugin needs no
...
export async function gameCoordinatorFor(env: Env, pluginName: string): Promise<GameCoordinatorConfig | false> {
  const config = await getGameLauncherConfig(env, pluginName);
  if (!config) {
    throw new Error(`No game-launcher config for "${pluginName}" - add one via POST /admin/game-launchers`);
  }
